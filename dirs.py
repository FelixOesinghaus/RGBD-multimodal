...
def create_depthcrop_directory(paths, dirs, files):
	main_crop_path = paths + "\\depthcrop"
	os.mkdir(main_crop_path)
	print("paths: ", paths)
	print("dirs: ", dirs)
	print("files: ", files)
	...
	for directory in dirs:
		...
		for subdirectory in subdirs:
			...
			object_pics = []
			for file in objectfiles:
				if "_depthcrop.png" in file:
					picpath = sub_sub_path + "\\" + file
					object_pics.append(picpath)
			print(len(object_pics))
			current_path = "rgbd-dataset\\" + directory + "\\" + subdirectory
			new_path = "depthcrop\\" + directory
			for pic in object_pics:
				if "_depthcrop.png" in pic:
					Path(pic).rename(pic.replace(current_path, new_path, 1))
...
# if depthcrop does not exist, use create_depthcrop_directory
if Path.exists(Path(os.getcwd() + "\\depthcrop")):
	print("./depthcrop existiert bereits.")
else:
	create_depthcrop_directory(paths, dirs, files)
...
data_dir = pathlib.Path(crop_path)
